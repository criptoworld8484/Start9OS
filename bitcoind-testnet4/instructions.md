...
> **This package always runs on Bitcoin's testnet4 network**, not mainnet. It is
> a forked build for testing/development. Its package id is `bitcoind`, so it
> acts as a drop-in replacement that dependent apps (Fulcrum, electrs, etc.)
> recognize as Bitcoin Core. Because it shares that id, it **cannot be installed
> alongside the official mainnet `bitcoind`** — install one or the other. All
> data lives under the `testnet4/` subdirectory and the node selects testnet4
> automatically — there is no network selector.
