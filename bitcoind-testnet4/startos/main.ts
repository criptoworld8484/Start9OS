...
export const main = sdk.setupMain(async ({ effects }) => {
  ...
  // Seleccionar testnet4. Los binds (rpcbind/bind/whitebind) y ZMQ ya vienen
  // del bitcoin.conf del paquete y Bitcoin Core los aplica a la red activa.
  bitcoinArgs.push(...testnet4LaunchArgs())
  ...
  const base = sdk.Daemons.of(effects)
    ...
    // On testnet4 the RPC cookie lives in ${rootDir}/${networkSubdir}/.cookie,
    // but dependents (Fulcrum, electrs, …) mount this volume and read the
    // cookie at its root (/mnt/bitcoind/.cookie). Expose it there via a
    // RELATIVE symlink so it also resolves inside the dependent's mount
    // namespace.
    .addOneshot('cookie-root-link', {
      subcontainer: bitcoindSub,
      exec: {
        command: [
          'sh',
          '-c',
          `ln -sfn ${networkSubdir}/.cookie ${rootDir}/.cookie`,
        ],
      },
      requires: [],
    })
  ...
  const withBitcoind = await base
    .addDaemon('bitcoind', {
      ...
      requires: ['nocow', 'clean-chainstate-old', 'cookie-root-link'],
    })
