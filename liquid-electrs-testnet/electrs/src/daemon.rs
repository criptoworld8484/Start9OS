...
impl Daemon {
    ...
    fn get_all_headers(&self, tip: &BlockHash) -> Result<Vec<BlockHeader>> {
        ...
        // Smaller chunks bound the transient memory of the initial header download:
        // each chunk is fetched as one batched JSON-RPC call (100k headers at once
        // spiked electrs — and the node serving the batch — to ~3.3 GB and OOM-killed
        // the indexer on a 6 GB box). 10k keeps the per-chunk JSON well bounded.
